...
class MvcBootstrap extends AutoBootstrap
{
    ...
    public function __construct($appPath)
    {
        ...
        $resources = array('router', 'request', 'response', 'dispatcher', 'view', 'dao');
...
//        echo"<pre>";print_r($resources);die($appPath);
...
//        $cls = new ArrayList();
//        $cls = new Card();
        //$this->_recurseDirectories( APPLICATION_PATH . '/../library/Micro/Dao/generated/classes/' );
    }

    private function _recurseDirectories($dirs)
    {
        if ($handle = opendir($dirs)) {
            while (false !== ($entry = readdir($handle))) {
                if ($entry != "." && $entry != "..") {
                    if (is_dir($dirs . "/" . $entry)) {
                        $this->_recurseDirectories($dirs . "/" . $entry);
                    } else {
                        $cls = new $entry();
//                        echo "$entry\n";
                    }
